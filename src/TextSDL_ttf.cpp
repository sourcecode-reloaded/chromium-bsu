...
{
	char save;
	if(len >= 0){ save = str[len]; str[len] = '\0'; }
	TTF_RenderUTF8_Blended(font, str_);
	if(len >= 0) str[len] = save;
}
...
float TextSDL_ttf::Advance(const char* str, const int len)
{
	...
	char save;
	if(len >= 0){ save = str[len]; str[len] = '\0'; }
	TTF_SizeUTF8(font, str_, &width, NULL);
	if(len >= 0) str[len] = save;
	return (float)height;
}
...
float TextSDL_ttf::LineHeight(const char* str, const int len)
{
	...
	char save;
	if(len >= 0){ save = str[len]; str[len] = '\0'; }
	TTF_SizeUTF8(font, str_, NULL, &height);
	if(len >= 0) str[len] = save;
	return (float)height;
}
